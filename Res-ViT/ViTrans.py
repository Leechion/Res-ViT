...
class ResNet_ViT(nn.Module):
    def __init__(self, num_classes=10, vit_patch_size=16, vit_dim=768, vit_depth=12, vit_heads=12):
        ...
        self.resnet = nn.Sequential(*list(self.resnet.children())[:-1])
        # 定义ViT模型
        self.vit = ViT(image_size=64, patch_size=4, num_classes=vit_dim, channels=3,
...
# #
# # train_data_set = RetinaMNIST(split="train", root=data_dir, transform=transform_mnist, download=True)
# # val_data_set = RetinaMNIST(split="val", root=data_dir, transform=transform_mnist, download=True)
# #
# # train_loader = DataLoader(train_data_set, batch_size=16, shuffle=True, num_workers=4)
# # val_loader = DataLoader(val_data_set, batch_size=16, shuffle=True, num_workers=4)
# #
#
#
...
# '''
# patch大小为 2x2（对于 32x32图像，这意味着每个图像 16x 16 = 196 个patch）、10 个可能的目标类别（0 到 9）和 1 个颜色通道（因为图像是灰度）。
# 在网络参数方面，使用了 64 个单元的维度，6 个 Transformer 块的深度，8 个 Transformer注意力机制头，MLP 使用 128 维度。'''
